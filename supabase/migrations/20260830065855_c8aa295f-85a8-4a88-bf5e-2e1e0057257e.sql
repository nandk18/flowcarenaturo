  ...
  WITH leads AS (
    SELECT p.id,
      ...
      WHERE (p_clinic_id IS NULL OR p.clinic_id = p_clinic_id)
        ...
        AND (p.lead_status <> 'current' OR nullif(trim(p.lead_source), '') IS NOT NULL)
  ...
  SELECT coalesce(jsonb_agg(r), '[]'::jsonb)
    ...
    FROM (
      SELECT jsonb_build_object(
        ...
        FROM public.patients p
       WHERE (p_clinic_id IS NULL OR p.clinic_id = p_clinic_id)
         AND p.lead_status IN ('attempt1','attempt2','attempt3','closed','lapsed')
         AND (
              p.lead_status IN ('attempt1','attempt2','attempt3')
              OR (p.created_at AT TIME ZONE 'Asia/Kolkata')::date BETWEEN p_from AND p_to
             )
       ORDER BY p.call_due_date NULLS LAST, p.created_at DESC
       LIMIT 300
    ) q;
