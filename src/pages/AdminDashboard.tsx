...
import { Stethoscope, Clock, AlertTriangle, ArrowRight } from "lucide-react";
...
function TodaysConsultations() {
  ...
  const statusColor = (s: string) =>
    s === "waiting" ? "bg-warning/15 text-warning border-warning/30" :
    s === "in_progress" ? "bg-info/15 text-info border-info/30" :
    s === "completed" ? "bg-success/15 text-success border-success/30" :
    "bg-muted text-muted-foreground";

  const getAge = (dob: string | null) => dob ? Math.floor((Date.now() - new Date(dob).getTime()) / (365.25 * 24 * 60 * 60 * 1000)) : null;
  ...
  const active = visits.filter(v => v.status === "waiting" || v.status === "in_progress");
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <Card key={visit.id} className="shadow-card hover:shadow-elevated cursor-pointer" onClick={() => handleStart(visit)}>
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                {visit.patient && (
                  <PatientLink patientId={visit.patient.id} className="text-foreground truncate">{visit.patient.name}</PatientLink>
                )}
            ...
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              ...
              <Button size="sm" className="text-xs" onClick={(e) => { e.stopPropagation(); handleStart(visit); }}>
                <ArrowRight className="mr-1 h-3 w-3" /> {visit.status === "waiting" ? "Start" : "Continue"}
              </Button>
            </div>
          </CardContent>
        </Card>
